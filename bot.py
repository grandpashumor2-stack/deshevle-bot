...
def init_db():
    ...
    con.executescript("""
        CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY, username TEXT DEFAULT '');
...
def main_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔥 Горящие скидки",   callback_data="hot"),
         InlineKeyboardButton("ℹ️ Как работает",     callback_data="info")],
    ])

back_btn = lambda: InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="menu")]])
...
async def start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_text(
        "👋 Привет! Слежу за ценами на Wildberries.\n\n"
...
async def adm_stats(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_text(f"📊 *Статистика:*\n\n👥 {s['users']} пользователей\n"
        f"👁 {s['watches']} отслеживаний\n🔔 {s['notifications']} уведомлений", parse_mode="Markdown")
...
async def btn(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if d=="menu":
        ...
        await q.edit_message_text(
            ...
            "2️⃣ Введи название\n3️⃣ Укажи макс. цену\n"
            "4️⃣ Бот проверяет WB каждые 30 мин\n"
            "5️⃣ Находит дешевле — пишет тебе!\n\n"
            "🔗 Ссылки партнёрские — покупаешь по той же цене, бот получает %",
            parse_mode="Markdown", reply_markup=back_btn())

    elif d=="add":
        if watches_count(uid)>=3:
            await q.edit_message_text("⚠️ Лимит 3 отслеживания. Удали одно.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📋 Список", callback_data="list")]])); return
        c.user_data["s"]=ENTER_QUERY
        await q.edit_message_text("🔍 Напиши название товара:\n\n_Например: кроссовки Nike_",
            parse_mode="Markdown",
        ...
        await q.edit_message_text(txt, parse_mode="Markdown", disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Обновить",callback_data="hot"),
                                               InlineKeyboardButton("◀️ Назад",callback_data="menu")]]))
...
async def msg(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if s==ENTER_QUERY:
        ...
        await m.edit_text(reply, parse_mode="Markdown", disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📋 Список",callback_data="list"),
...
def run_web():
    HTTPServer(("0.0.0.0",PORT),H2).serve_forever()
