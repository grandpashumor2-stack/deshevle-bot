...
RENDER_URL = os.environ.get("RENDER_URL", "")  # https://deshevle-bot.onrender.com
...
# ─── Пинг себя каждую минуту чтобы не засыпать ───────────────────────────────

async def keep_alive():
    """Пингует сам себя каждые 60 секунд — Render не засыпает."""
    await asyncio.sleep(30)  # ждём пока сервер поднимется
    while True:
        try:
            if RENDER_URL:
                async with aiohttp.ClientSession() as session:
                    async with session.get(RENDER_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
...
async def health(request):
    return web.Response(text="OK — DeshevleBot is alive!")
...
async def main():
    await db.init()
    await run_web()
    asyncio.create_task(keep_alive())       # пинг каждую минуту
    await start_scheduler(bot, db, parser)  # проверка цен каждые 30 мин
