"""
Парсер Wildberries с защитой от rate limit (HTTP 429) и сбоев JSON.
- Пауза между запросами
- Экспоненциальный backoff при 429
- Retry при ошибках парсинга JSON (обрезанный ответ)
...
# Время последнего запроса — не чаще 1 раза в 2.5 секунды
_last_request_time = 0
MIN_INTERVAL = 2.5  # секунды между запросами
...
def _headers():
    return {
        ...
        "Accept-Encoding": "identity",  # без сжатия — меньше шанс обрезанного ответа
        "Origin": "https://www.wildberries.ru",
        "Referer": "https://www.wildberries.ru/",
        "Connection": "close",  # закрываем соединение — без переиспользования "битых" коннектов
    }
...
def _wb_get(params: dict, retries: int = 3) -> list:
    """
    Один запрос к WB с retry при 429 и при ошибках JSON.
        ...
        try:
            ...
            if r.status_code == 200:
                try:
                    ...
                    # Обрезанный/битый ответ — повторяем запрос
                ...
                wait_time = 5 * (attempt + 1)  # 5с, 10с, 15с, 20с
